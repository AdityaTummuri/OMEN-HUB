...
2. **D-Bus Communication:**
   - Like all client components in OMEN-HUB, the CLI runs completely unprivileged. 
   - It acts as a synchronous/asynchronous DBus client, forwarding arguments to the `omen-hub-daemon` (or legacy `omen-space-daemon`).
