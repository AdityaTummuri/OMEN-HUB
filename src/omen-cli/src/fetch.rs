...
use crate::dbus_proxy::{FanProxy, MuxProxy, PlatformProxy, PowerProxy, RgbProxy, SysMonProxy};
...
pub async fn run_live_dashboard(conn: &zbus::Connection) -> Result<()> {
    ...
    // Initial state fetch
    let sys_json: Value =
        serde_json::from_str(&platform.get_hardware_dump_json().await.unwrap_or_default())
            .unwrap_or(Value::Null);
    let mut fan_json: Value =
        serde_json::from_str(&fan.get_fan_info().await.unwrap_or_default()).unwrap_or(Value::Null);
    let mut power_json: Value =
        serde_json::from_str(&power.get_power_profile().await.unwrap_or_default())
            .unwrap_or(Value::Null);
    let mut rgb_json: Value =
        serde_json::from_str(&rgb.get_state().await.unwrap_or_default()).unwrap_or(Value::Null);
    let mut state_json: Value =
        serde_json::from_str(&platform.get_state().await.unwrap_or_default())
            .unwrap_or(Value::Null);
    let conflicts_json: Value =
        serde_json::from_str(&platform.check_conflicts().await.unwrap_or_default())
            .unwrap_or(Value::Null);
    let mut sysmon_json: Value =
        serde_json::from_str(&sysmon.get_diagnostics().await.unwrap_or_default())
            .unwrap_or(Value::Null);
    ...
    loop {
        if need_redraw {
            ...
            let user_host_header = format!(
                "\x1b[1;38;2;0;200;255m{}\x1b[0m@\x1b[1;38;2;180;0;255m{}\x1b[0m",
                user, hostname
            );
            ...
            write!(
                out,
                "\x1b[1;31momen-cli\x1b[0m \x1b[1;32m>\x1b[0m {}\x1b[K",
                input
            )?;
            out.flush()?;
        }
                ...
                match key.code {
                    ...
                    KeyCode::Enter => {
                        ...
                        if !cmd_str.is_empty() {
                            ...
                            fan_json =
                                serde_json::from_str(&fan.get_fan_info().await.unwrap_or_default())
                                    .unwrap_or(Value::Null);
                            power_json = serde_json::from_str(
                                &power.get_power_profile().await.unwrap_or_default(),
                            )
                            .unwrap_or(Value::Null);
                            rgb_json =
                                serde_json::from_str(&rgb.get_state().await.unwrap_or_default())
                                    .unwrap_or(Value::Null);
                            state_json = serde_json::from_str(
                                &platform.get_state().await.unwrap_or_default(),
                            )
                            .unwrap_or(Value::Null);
                            sysmon_json = serde_json::from_str(
                                &sysmon.get_diagnostics().await.unwrap_or_default(),
                            )
                            .unwrap_or(Value::Null);
                            need_redraw = true;
                        }
        ...
        if last_tick.elapsed() >= Duration::from_millis(1000) {
            ...
            fan_json = serde_json::from_str(&fan.get_fan_info().await.unwrap_or_default())
                .unwrap_or(Value::Null);
            power_json = serde_json::from_str(&power.get_power_profile().await.unwrap_or_default())
                .unwrap_or(Value::Null);
            rgb_json = serde_json::from_str(&rgb.get_state().await.unwrap_or_default())
                .unwrap_or(Value::Null);
            state_json = serde_json::from_str(&platform.get_state().await.unwrap_or_default())
                .unwrap_or(Value::Null);
            sysmon_json = serde_json::from_str(&sysmon.get_diagnostics().await.unwrap_or_default())
                .unwrap_or(Value::Null);
            need_redraw = true;
        }
    }
...
async fn execute_input_command(input: &str, conn: &zbus::Connection) -> String {
    ...
    match parts[0].to_lowercase().as_str() {
                ...
                if sub == "auto" || sub == "ec" || sub == "max" || sub == "custom" {
                    match f.set_fan_mode(&sub).await {
                        Ok(_) => format!(
                            "[{}] \x1b[1;32m{}\x1b[0m",
                            now,
                            crate::i18n::t("fan_changed").replace("{}", &sub.to_uppercase())
                        ),
                        Err(e) => format!(
                            "[{}] \x1b[1;31m{} {}\x1b[0m",
                            now,
                            crate::i18n::t("fan_error"),
                            e
                        ),
                    }
                    ...
                    if r1.is_ok() || r2.is_ok() {
                        format!(
                            "[{}] \x1b[1;32m{}\x1b[0m",
                            now,
                            crate::i18n::t("fan_set")
                                .replacen("{}", &val.min(100).to_string(), 1)
                                .replacen("{}", &rpm.to_string(), 1)
                        )
                    } else {
                        format!(
                            "[{}] \x1b[1;31m{}\x1b[0m",
                            now,
                            crate::i18n::t("fan_set_failed")
                        )
                    }
                } else {
                    format!(
            ...
            if let Some(p) = power {
                if sub == "performance" || sub == "perf" {
                    match p.set_power_profile("performance").await {
                        Ok(_) => {
                            format!("[{}] \x1b[1;32m{}\x1b[0m", now, crate::i18n::t("perf_perf"))
                        }
                        Err(e) => format!(
                            "[{}] \x1b[1;31m{} {}\x1b[0m",
                            now,
                            crate::i18n::t("perf_error"),
                            e
                        ),
                    }
                    ...
                    match p.set_power_profile("balanced").await {
                        Ok(_) => {
                            format!("[{}] \x1b[1;32m{}\x1b[0m", now, crate::i18n::t("perf_bal"))
                        }
                        Err(e) => format!(
                            "[{}] \x1b[1;31m{} {}\x1b[0m",
                            now,
                    ...
                    match p.set_power_profile("power-saver").await {
                        Ok(_) => {
                            format!("[{}] \x1b[1;32m{}\x1b[0m", now, crate::i18n::t("perf_eco"))
                        }
                        Err(e) => format!(
                            "[{}] \x1b[1;31m{} {}\x1b[0m",
                            now,
                    ...
                    match p.set_power_limits(true, pl1, pl2).await {
                        Ok(_) => format!(
                            "[{}] \x1b[1;32m{}\x1b[0m",
                            now,
                            crate::i18n::t("perf_limits")
                                .replacen("{}", &pl1.to_string(), 1)
                                .replacen("{}", &pl2.to_string(), 1)
                        ),
                ...
                if let Some(p) = platform {
                    match p.set_battery_care(limit).await {
                        Ok(_) => format!(
                            "[{}] \x1b[1;32m{}\x1b[0m",
                            now,
                            crate::i18n::t("bat_set").replacen(
                                "{}",
                                &limit.clamp(50, 100).to_string(),
                                1
                            )
                        ),
                    ...
                    format!(
                        "[{}] \x1b[1;31m{}\x1b[0m",
                        now,
                        crate::i18n::t("bat_no_service")
                    )
                }
            } else {
                format!(
                    "[{}] \x1b[1;31mInvalid battery limit (expected 50-100)\x1b[0m",
                    now
                )
            }
        }
        ...
        "uv" | "undervolt" => {
            if parts.len() < 2 {
                return format!(
                    "[{}] \x1b[33mUsage: uv <mv_offset> (e.g. uv -50)\x1b[0m",
                    now
                );
            }
                ...
                if let Some(p) = power {
                    match p.set_undervolt(mv).await {
                        Ok(_) => format!(
                            "[{}] \x1b[1;32m{}\x1b[0m",
                            now,
                            crate::i18n::t("uv_set").replacen("{}", &mv.to_string(), 1)
                        ),
                        Err(e) => format!(
                            "[{}] \x1b[1;31m{} {}\x1b[0m",
                            now,
                            crate::i18n::t("uv_error"),
                            e
                        ),
                    }
                } else {
        ...
        "rgb" => {
            if parts.len() < 2 {
                return format!(
                    "[{}] \x1b[33mUsage: rgb on | off | red | blue | green | white | hex\x1b[0m",
                    now
                );
            }
            ...
            if let Some(r) = rgb {
                if color_arg == "off" {
                    match r.set_global(false, 0, "right").await {
                        Ok(_) => {
                            format!("[{}] \x1b[1;32m{}\x1b[0m", now, crate::i18n::t("rgb_off"))
                        }
                        Err(e) => format!(
                            "[{}] \x1b[1;31m{} {}\x1b[0m",
                            now,
                            crate::i18n::t("rgb_error"),
                            e
                        ),
                    }
                    ...
                    match r.set_color(8, hex).await {
                        Ok(_) => format!(
                            "[{}] \x1b[1;32m{}\x1b[0m",
                            now,
                            crate::i18n::t("rgb_color").replacen("{}", &hex, 1)
                        ),
                        Err(e) => format!(
                            "[{}] \x1b[1;31m{} {}\x1b[0m",
                            now,
            ...
            if let Some(p) = platform {
                match p.generate_triage_bundle().await {
                    Ok(path) => format!(
                        "[{}] \x1b[1;32m{}\x1b[0m",
                        now,
                        crate::i18n::t("diag_bundle").replacen("{}", &path, 1)
                    ),
                    ...
                    if let Some(cmd) = &parsed_cli.command {
                        match run_command(cmd, conn).await {
                            Ok(_) => format!(
                                "[{}] \x1b[1;32m{}\x1b[0m",
                                now,
                                crate::i18n::t("executed").replacen("{}", input, 1)
                            ),
                            Err(e) => format!(
                                "[{}] \x1b[1;31mError executing '{}': {}\x1b[0m",
                                now, input, e
                            ),
                        }
                    } else {
