...
fn get_zone_for_key(name: &str) -> i32 {
    if name == "zone_0" || name == "c1_btn" {
        return 1;
    }
    if name == "global_color_btn" {
        return 8;
    }
    if name == "zone_1" || name == "c2_btn" {
        return 2;
    }
    if name == "zone_2" || name == "c3_btn" {
        return 3;
    }
    if name == "zone_3" || name == "c4_btn" {
        return 4;
    }
    match name {
        "W" | "A" | "S" | "D" => 4,
        "Esc" | "F1" | "F2" | "F3" | "F4" | "~" | "1" | "2" | "3" | "4" | "Tab" | "Q" | "E"
        | "R" | "Caps" | "F" | "Shift" | "Z" | "X" | "C" | "V" | "Ctrl" | "Win" => 1,
        "F5" | "F6" | "F7" | "F8" | "5" | "6" | "7" | "8" | "T" | "Y" | "U" | "I" | "G" | "H"
        | "J" | "K" | "B" | "N" | "M" | "," | "Alt" | "Space" => 2,
        _ => 3,
    }
}
...
#[allow(deprecated)]
pub fn show_color_picker_popover(parent: &gtk::Button, on_color_selected: Rc<dyn Fn(String)>) {
    ...
    custom_btn.connect_clicked(move |_| {
        ...
        dialog.choose_rgba(
            parent_win_ref.as_ref(),
            None::<&gtk::gdk::RGBA>,
            None::<&gtk::gio::Cancellable>,
            move |res: Result<gtk::gdk::RGBA, glib::Error>| {
                if let Ok(rgba) = res {
                    let hex = format!(
                        "#{:02X}{:02X}{:02X}",
                        (rgba.red() * 255.) as u8,
...
fn build_interactive_keyboard(
    ...
    let buttons_map: Rc<RefCell<HashMap<String, gtk::Button>>> =
    ...
    for row_keys in &layout {
        ...
        for (name, size_mult) in row_keys {
            ...
            buttons_map
                .borrow_mut()
                .insert(name.to_string(), key_btn.clone());
    ...
    let global_color_box = gtk::Box::builder()
        .orientation(gtk::Orientation::Horizontal)
        .spacing(12)
        .margin_top(8)
        .margin_bottom(12)
        .halign(gtk::Align::Center)
        .build();
    let global_color_label = gtk::Label::builder()
        .label(i18n::t("kb_global_color"))
    ...
    let global_color_btn = gtk::Button::builder()
        .width_request(40)
        .height_request(24)
        .build();
    ...
    let c1_label = gtk::Label::builder()
        .label(i18n::t("kb_color_1"))
        .css_classes(["dim-label"])
        .build();
    let c1_btn = gtk::Button::builder()
        .width_request(40)
        .height_request(24)
        .tooltip_text(i18n::t("kb_color_1"))
        .build();
    c1_btn.add_css_class("circular");
    c1_btn.set_widget_name("c1_btn");

    let c2_label = gtk::Label::builder()
        .label(i18n::t("kb_color_2"))
        .css_classes(["dim-label"])
        .build();
    let c2_btn = gtk::Button::builder()
        .width_request(40)
        .height_request(24)
        .tooltip_text(i18n::t("kb_color_2"))
        .build();
    c2_btn.add_css_class("circular");
    c2_btn.set_widget_name("c2_btn");

    let c3_label = gtk::Label::builder()
        .label(i18n::t("kb_color_3"))
        .css_classes(["dim-label"])
        .build();
    let c3_btn = gtk::Button::builder()
        .width_request(40)
        .height_request(24)
        .tooltip_text(i18n::t("kb_color_3"))
        .build();
    c3_btn.add_css_class("circular");
    c3_btn.set_widget_name("c3_btn");

    let c4_label = gtk::Label::builder()
        .label(i18n::t("kb_color_4"))
        .css_classes(["dim-label"])
        .build();
    let c4_btn = gtk::Button::builder()
        .width_request(40)
        .height_request(24)
        .tooltip_text(i18n::t("kb_color_4"))
        .build();
    ...
    buttons_map
        .borrow_mut()
        .insert("global_color_btn".to_string(), global_color_btn.clone());
    buttons_map
        .borrow_mut()
        .insert("c1_btn".to_string(), c1_btn.clone());
    buttons_map
        .borrow_mut()
        .insert("c2_btn".to_string(), c2_btn.clone());
    buttons_map
        .borrow_mut()
        .insert("c3_btn".to_string(), c3_btn.clone());
    buttons_map
        .borrow_mut()
        .insert("c4_btn".to_string(), c4_btn.clone());
    ...
    let on_color_picked_mode_update = Rc::new(move |mode_val: KeyboardMode| {
        if let Some(row) = &*mode_row_ref_cb.borrow() {
            let idx = if mode_val == KeyboardMode::PerKey {
                1
            } else {
                0
            };
                ...
                Rc::new(move |hex| {
                    ...
                    match current_mode {
                        KeyboardMode::PerKey => {
                            ...
                            kc_local
                                .borrow_mut()
                                .insert(name_local.clone(), hex.clone());
                            crate::daemon_client::set_per_key_colors_sync(
                                pk_local.borrow().clone(),
                            );
                        }
                        KeyboardMode::Victus1Zone => {
                            zc_local.borrow_mut()[0] = hex.clone();
    ...
    let apply_anim = Rc::new(move |mode: &str, speed: f64| {
        ...
        match mode {
            "static" => {
                for (k, c) in kc_anim.borrow().iter() {
                    if let Some(btn) = b_map_anim.borrow().get(k) {
                        css.push_str(&format!(
                            "#{} {{ background: {}; animation: none; opacity: 1.0; }}\n",
                            btn.widget_name(),
                            c
                        ));
                    }
            ...
            "breathing" => {
                css.push_str(
                    "@keyframes kb_breathe { 0%, 100% { opacity: 0.1; } 50% { opacity: 1.0; } }\n",
                );
                for (k, c) in kc_anim.borrow().iter() {
                    if let Some(btn) = b_map_anim.borrow().get(k) {
...
fn build_interactive_lightbar(state_json_opt: &Option<serde_json::Value>) -> gtk::Box {
    ...
    let mut initial_colors = default_colors
        .iter()
        .map(|s| s.to_string())
        .collect::<Vec<_>>();
...
pub fn build_page() -> (
    ...
    let detected_mode = match zone_override {
        ...
        _ => {
            ...
            if specs.product_name.to_lowercase().contains("desktop")
                || specs.product_name.to_lowercase().contains("tower")
            {
                KeyboardMode::DesktopRgb
            } else if is_per_key {
                KeyboardMode::PerKey
    ...
    let badge = gtk::Label::builder()
        .label(if detected_mode == KeyboardMode::DesktopRgb {
            "Desktop"
        } else if is_per_key {
            "Per-Key"
        } else if is_omen {
    ...
    let effect_speed_row = adw::ActionRow::builder()
        .title(i18n::t("kb_effect_speed"))
        .build();
    ...
    let (std_kb_grid, apply_anim, global_color_box_ref) =
        if detected_mode == KeyboardMode::DesktopRgb {
    ...
    *apply_anim_rc.borrow_mut() = Some(apply_anim.clone());
    ...
    if is_omen_brand {
        ...
        let lb_bright_row = adw::ActionRow::builder()
            .title(i18n::t("lightbar_brightness"))
            .build();
        ...
        let mut show_lightbar = detected_mode == KeyboardMode::DesktopRgb
