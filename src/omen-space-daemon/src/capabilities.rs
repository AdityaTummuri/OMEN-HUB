...
impl LinuxCapabilityClassifier {
    pub fn assess(is_root: bool, board_id: &str, _model: &str) -> LinuxCapabilityAssessment {
        ...
        let has_thermal_profile = Path::new("/sys/devices/platform/hp-wmi/thermal_profile")
            .exists()
            || Path::new("/sys/devices/platform/hp-wmi/thermal-profile").exists();
        let has_platform_profile = Path::new("/sys/devices/platform/hp-wmi/platform_profile")
            .exists()
        ...
        let has_profile_control = has_thermal_profile
            || has_platform_profile
            || has_acpi_platform_profile
            || has_hwmon_fan_access;
        ...
        if is_wmaa_abort_prone && has_manual_fan_control {
            ...
            return LinuxCapabilityAssessment {
                capability_class: if has_profile_control {
                    LinuxCapabilityClass::ProfileOnly
                } else {
                    LinuxCapabilityClass::TelemetryOnly
                },
